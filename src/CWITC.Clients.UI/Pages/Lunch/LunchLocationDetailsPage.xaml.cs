                ...
                else if (Device.OS == TargetPlatform.iOS)
                {
                    ToolbarItems.Add(new ToolbarItem
                    {
                        ...
                        Command = new Command(async () =>
                        {
                            ...
                            if (!vm.CanMakePhoneCall)
                            {
                                items = new[] { Language.ViewMenu, Language.GetDirections };
                            }
                            else
                            {
                                items = new[] { Language.ViewMenu, Language.GetDirections, string.Format(Language.CallLocationFormat, vm.Location.Name) };
                            }
                            ...
                            else if (items.Length > 2 && action == items[2] && vm.CanMakePhoneCall)
